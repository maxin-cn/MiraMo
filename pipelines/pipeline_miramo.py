...
class MiraMoPipeline(DiffusionPipeline, SanaLoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        if use_resolution_binning:
            if self.transformer.config.sample_size == 128:
                ...
                aspect_ratio_bin = ASPECT_RATIO_480P
            else:
        ...
        motion_bucket_id = torch.tensor(motion_bucket_id).to(latents.device, dtype=latents.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                motion_bins_input = motion_bucket_id.expand(latent_model_input.shape[0])
                timestep = timestep * self.transformer.config.timestep_scale

                # predict noise model_output
                noise_pred = self.transformer(
                    latent_model_input,
                    motion_bins=motion_bins_input,
        ...
        # if not output_type == "latent": # do this in decode_latents
